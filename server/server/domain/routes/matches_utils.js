...
function compareDates(d1, d2){
    ...
    if(parts_d1[1]<0 || parts_d1[1]>31 || parts_d2[1]<0 || parts_d2[1]>31)
      return;
    //year
    if(parts_d1[2]<0|| parts_d2[2]<0)
      return;
    console.log( "year1 "+parts_d1[2])
    console.log( "year2 "+parts_d2[2])
    if(parts_d1[2]>parts_d2[2])
        return false;
    else if(parts_d1[2]<parts_d2[2])
        return true;

    if(parts_d1[0]>parts_d2[0])
      return false;
    else if(parts_d1[0]<parts_d2[0])
      return true;

    if(parts_d1[1]>parts_d2[1])
      return false;
    return true;
      }
...
async function getPastGames(teamName){
    ...
    for (let i=0 ; i<matches.length; i++){
      if( matches[i].awayTeam!=teamName && matches[i].homeTeam!=teamName)
        continue;
      let date = matches[i].date;     
      date = date_to_string( date )
      if (compareDates(date,today)==true) 
...
async function getFutureGames(teamName){
    ...
    for (let i=0 ; i<matches.length; i++){
      ...
      let date = matches[i].date;     
      date = date_to_string( date )
      if (compareDates(date,today)==false) 
        result.push(matches[i]);
